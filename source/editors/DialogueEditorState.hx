...
class DialogueEditorState extends MusicBeatState
{
	...
	var voiceInputText:FlxUIInputText;
	...
	function addDialogueLineUI() {
		...
		voiceInputText = new FlxUIInputText(10, speedStepper.y + 40, 150, '', 8);
		blockPressWhileTypingOn.push(voiceInputText);
		
		lineInputText = new FlxUIInputText(10, voiceInputText.y + 35, 200, DEFAULT_TEXT, 8);
		...
		tab_group.add(new FlxText(10, voiceInputText.y - 18, 0, 'Sound file name:'));
	...
	function copyDefaultLine():DialogueLine
	{
		var copyLine:DialogueLine = {
			...
			voiceLine: defaultLine.voiceLine
		};
		return copyLine;
	}
	...
	function reloadText(speed:Float = 0.05) {
		...
		Alphabet.setDialogueSound(voiceInputText.text);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			else if(sender == voiceInputText)
			{
				dialogueFile.dialogue[curSelected].voiceLine = voiceInputText.text;
				reloadText(0);
			}
