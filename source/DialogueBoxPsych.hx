...
class DialogueBoxPsych extends FlxSpriteGroup
{
	...
	function startNextDialog():Void
	{
		...
		if (curDialogue.voiceLine != null && curDialogue.voiceLine != "")
			{
					var voiceLine:String = voicePrefix + curDialogue.voiceLine; 
					trace(voiceLine);

					if (Paths.fileExists('sounds/' + voiceLine + '.ogg', SOUND, true))
					{
						if (curSound != null && curSound.playing)
							{
								...
								curSound = new FlxSound().loadEmbedded(Paths.sound(voiceLine)); 
								curSound.play();
								trace('should be playing sound'); 
								daText.dialogueSound = null; 
								daText.soundOverride = true;
					}
					else
								trace('couldnt find file lol');
			}
			else
			{
					trace("no line");
